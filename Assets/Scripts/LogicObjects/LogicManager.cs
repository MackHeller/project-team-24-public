...
public class LogicManager {
    ...
    public void run() {
        ...
        for (int i = 0; i < inputs.Count; i++) {
            inputs[i].setInput(inputLogic[i]);
        }
    }
    ...
    public bool checkIfOutputIsCorrect() {
        ...
        for (int i = 0; i < outputs.Count; i++) {
            if (outputs[i].getOutput() != expectedOutputLogic[i]) {
                return false;
            }
        }
        return true;
    ...
    public LogicModule addModule(LogicModule module) {
    ...
    public LogicModule getModuleAtId(int moduleId) {
        if (modules.ContainsKey(moduleId)) {
            return (LogicModule)modules[moduleId];
        } else
            throw new ArgumentException();
    }
